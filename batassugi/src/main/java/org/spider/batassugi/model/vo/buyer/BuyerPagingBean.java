...
/**
 * 페이징 처리를 위한 비즈니스 계층의 클래스 PagingBean method 구현순서 getStartRowNumber() getEndRowNumber().
...
public class BuyerPagingBean {
  ...
  public BuyerPagingBean() {}

  public BuyerPagingBean(int totalContents) {
    this.totalContents = totalContents;
  }

  public BuyerPagingBean(int nowPage, int totalContents) {
    this.totalContents = totalContents;
  ...
  /**
   * 현재 페이지번호에 해당하는 시작 게시물의 row number를 반환 hint : 이전페이지의 마지막 번호 + 1 ((현재페이지-1) * 페이지당 게시물수) +1.
   * 
   * @return
   */
